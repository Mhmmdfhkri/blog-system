...
// Custom Parser dengan Toggle Support
const customParserWithToggle = (blocks) => {
  const html = [];
  ...
  for (let i = 0; i < blocks.length; i++) {
    ...
    if (block.type === "toggle") {
      ...
      // Ambil block setelah toggle sebagai kontennya (jika ada)
      const nextBlock = blocks[i + 1];
      let content = "";

      if (nextBlock && nextBlock.type === "paragraph") {
        content = nextBlock.data.text || "";
        i++; // lewati blok yang sudah dipakai
      }
    ...
    // Gunakan parser lain jika bukan toggle
    else if (customParsers[block.type]) {
      html.push(customParsers[block.type](block));
    }
  }
            ...
            ${
              showlinenumbers
                ? code
                    ...
                    .map(
                      (line, idx) =>
                        `<div class="flex"><span class="pr-4 text-gray-400 select-none">${idx + 1}</span><span>${line
                          .replace(/</g, "&lt;")
                          .replace(/>/g, "&gt;")}</span></div>`
                    )
                    .join("")
  ...
  quote: (block) => {
    ...
    return `
      <blockquote class="border-l-4 border-blue-600 pl-4 italic text-gray-600 my-4">
        "${text}"
        ${caption ? `<footer class="mt-2 text-sm text-right">— ${caption}</footer>` : ""}
      </blockquote>
    `;
  },

  delimiter: () => `<div class="text-center text-4xl my-4 text-black">***</div>`,
...
function SingleBlogCard({ blog }) {
  ...
  // Gunakan custom parser dengan support toggle
  const htmlContent = content?.blocks ? customParserWithToggle(content.blocks) : "";
  ...
  return (
    <div className="flex flex-col gap-8 px-8 md:px-16">
      ...
      <div className="flex flex-col lg:flex-row gap-12">
        ...
        {/* RelatedBlogs bisa ditambahkan jika kamu punya */}
      </div>
    </div>
  );
}
